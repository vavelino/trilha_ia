...
function productionBundle(react: ReasoningStrategy, extras?: {
  ...
  return {
    ...
    team: extras?.team ?? fakeStrategy({ name: "team" }),
  };
}
...
test("018/US4: strategy team accepted as override with handoffs in trace", async () => {
  const react = fakeStrategy({ name: "react" });
  const team = fakeStrategy({
    name: "team",
    run: async () => ({
  ...
  const app = testApp({ react, team, classifyRoute: defaultClassify("react") });

  await withServer(app, async (baseUrl) => {
    const { status, json } = await postChat(baseUrl, {
      message: "investigue, planeje e resolva",
      strategy: "team",
    });
    assert.equal(status, 200);
    assert.equal(json.answer, "equipe resolveu");
    assert.equal(react.calls, 0);
    const metrics = json.metrics as { route?: string };
    assert.equal(metrics.route, "team");
    const trace = json.trace as Array<{
      type: string;
      node?: string;
      to?: string;
      route?: string;
    ...
    assert.equal(routeEvent?.override, true);
    const handoffs = trace.filter((e) => e.type === "handoff");
    assert.equal(handoffs.length, 2);
    assert.ok(handoffs.every((e) => e.node === "supervisor"));
    assert.equal(handoffs[0]?.to, "analista");
    assert.equal(handoffs[0]?.content, "levante fatos");
  });
});

test("018/US4: awaitHumanApproval defers team turn without executing roles", async () => {
  const team = fakeStrategy({ name: "team" });
  const approvals = new MemoryApprovalStore();
  const app = testApp({ team, approvals });

  await withServer(app, async (baseUrl) => {
    const pending = await postChat(baseUrl, {
      message: "resolva o incidente inc-123",
      strategy: "team",
      awaitHumanApproval: true,
    });
    assert.equal(pending.status, 202);
    assert.equal(team.calls, 0);
    const approvalId = (pending.json.pending as { approvalId: string }).approvalId;
