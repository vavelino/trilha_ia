...
export function createProductionGraph(deps: ProductionGraphDeps) {
  ...
  const routerNode = async (state: GraphStateType) => {
    ...
    try {
      ...
      if (!isProductionRoute(decision.route)) {
  ...
  const runStrategy = async (
    ...
    return {
      ...
      trace: stampTrace ? stampNode(node, result.trace) : result.trace,
    };
  };
  ...
  // Team events are pre-signed per role (supervisor/analista/...) — do not re-stamp.
  const teamNode = (state: GraphStateType) =>
    runStrategy(state, deps.strategies.team, "team", false);
