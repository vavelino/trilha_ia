...
> Este é o model card de referência do curso, gerado rodando `model-versioning-tool.js` contra o job publicado pelo professor -- mostra o formato exato do que a ferramenta produz. Pra gerar o seu, rode o script com `TUNING_JOB_NAME` apontando pro job que você mesmo criou (veja README.md, seção "Antes de rodar").
