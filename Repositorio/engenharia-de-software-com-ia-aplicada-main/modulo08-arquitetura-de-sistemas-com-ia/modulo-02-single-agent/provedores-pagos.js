 ...
 *
 * Três alternativas PAGAS ao Ollama local de react-agent-prototype.js: Claude
 ...
 * ferramenta e o critério de parada de agenteICF() não mudam em nenhuma das três.
 *
 ...
 *         importar dentro do seu próprio protótipo. Rodar `node provedores-pagos.js`
 ...
 *
 * Existe uma versão idêntica em Python: provedores_pagos.py
 */

// ---------- 1. Claude (Anthropic) — npm install @anthropic-ai/sdk ----------
...
async function chamarClaude(historico) {
  ...
  return chamada
    ? { tipo: 'chamada_ferramenta', nome: chamada.name, args: chamada.input }
    : { tipo: 'resposta_final', texto: resposta.content[0].text };
}

// ---------- 2. Gemini (Google) — npm install @google/genai ----------
const { GoogleGenAI } = require('@google/genai');
const gemini = new GoogleGenAI({}); // lê GEMINI_API_KEY do ambiente
...
// Nota de assinatura: chamarClaude/chamarGPT recebem `historico` (o mesmo array de
...
async function chamarGemini(protocolo) {
  const interacao = await gemini.interactions.create({
    model: 'gemini-3.5-flash',
    input: protocolo,
    tools: [toolGemini],
  });
  const chamada = interacao.steps.find((s) => s.type === 'function_call');
  return chamada
    ? { tipo: 'chamada_ferramenta', nome: chamada.name, args: chamada.arguments }
    : { tipo: 'resposta_final', texto: interacao.output_text };
}

// ---------- 3. GPT (OpenAI) — npm install openai ----------
const OpenAI = require('openai');
const gpt = new OpenAI(); // lê OPENAI_API_KEY do ambiente
...
async function chamarGPT(historico) {
  const resposta = await gpt.chat.completions.create({
    model: 'gpt-5.6',
    messages: historico,
    tools: [toolGPT],
  });
  ...
  return chamada
    ...
    : { tipo: 'resposta_final', texto: resposta.choices[0].message.content };
}

module.exports = { chamarClaude, chamarGemini, chamarGPT };
